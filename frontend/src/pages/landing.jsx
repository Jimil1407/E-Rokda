...
export default function Landing() {
    ...
    return (
        <div className="min-h-screen bg-black font-['Inter'] text-white">
            <nav className="flex justify-between items-center px-12 py-6 fixed top-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-md border-b border-white/20">
                <div className="nav-brand">
                    <h1 className="text-3xl font-black text-white m-0 tracking-tight uppercase">E-ROKDA</h1>
                </div>
                <div className="flex gap-4">
                    <Link to="/signin" className="px-6 py-3 rounded-lg text-white border-2 border-white font-semibold text-sm uppercase tracking-wider transition-all duration-200 hover:bg-white hover:text-black no-underline hover:-translate-y-1 hover:shadow-lg">
                        SIGN IN
                    </Link>
                    <Link to="/signup" className="px-6 py-3 rounded-lg bg-[#12b981] text-white border-2 border-[#12b981] font-semibold text-sm uppercase tracking-wider transition-all duration-200 hover:bg-[#0ea371] hover:border-[#0ea371] hover:-translate-y-1 hover:shadow-lg no-underline">
                        GET STARTED
                    </Link>
                </div>
            </nav>

            <section className="flex items-center justify-center px-12 py-32 max-w-7xl mx-auto pt-42">
                <div className="text-center max-w-4xl">
                    <h1 className="text-8xl font-black leading-tight mb-8 tracking-tight">
                        The Future of
                    ...
                    <div className="flex gap-6 justify-center">
                        <Link to="/signup" className="px-10 py-5 rounded-lg bg-[#12b981] text-white border-2 border-[#12b981] font-bold text-lg uppercase tracking-wider transition-all duration-200 hover:bg-[#0ea371] hover:border-[#0ea371] hover:-translate-y-1 hover:shadow-2xl hover:shadow-[#12b981]/30 no-underline">
                            START YOUR JOURNEY
                        </Link>
                    </div>
                </div>
            ...
            <section className="px-12 py-20 max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {features.map((feature, index) => (
                        <div 
                            key={index} 
                            className={`bg-black text-white p-10 rounded-xl text-center transition-all duration-300 border-2 border-[#12b981] cursor-pointer hover:-translate-y-2 hover:shadow-2xl hover:shadow-[#12b981]/20 ${currentFeature === index ? 'scale-105' : ''}`}
                            onMouseEnter={() => setCurrentFeature(index)}
                        >
                            <div className="text-5xl mb-4">{feature.icon}</div>
                            <h3 className="text-2xl font-bold mb-3 text-[#12b981]">{feature.title}</h3>
